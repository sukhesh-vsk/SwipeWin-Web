...
const SubmitButton: React.FC = () => {
  ...
  return (
    <div className="mt-1 flex flex-col justify-center items-center">
      ...
      <button
        className={cx(
          "w-1/2 py-2 text-white font-semibold text-center rounded-xl",
          {
  ...
  return (
    <div className="bg-text p-2 pt-4 mb-1 rounded-md overflow-auto border border-solid">
      <div className="flex items-center justify-between text-sm mb-1 text-bg">
        <div className="">
          Betslip {items.length > 1 ? "Combo" : "Single"}{" "}
          {items.length ? `(${items.length})` : ""}
        </div>
        {Boolean(items.length) && <button onClick={clear}>Remove All</button>}
      </div>
      {Boolean(items.length) ? (
        <>
          <div className="overflow-x-auto no-scrollbar flex items-stretch space-x-2">
            {items.map((item) => {
              const {
                game: { gameId, startsAt, sportName, leagueName, participants },
                conditionId,
                outcomeId,
              } = item;

              const marketName = getMarketName({ outcomeId });
              const selection = getSelectionName({
                outcomeId,
                withPoint: true,
              });
              ...
              const isLock =
                !isStatusesFetching &&
                statuses[conditionId] !== ConditionStatus.Created;

              return (
                <div
                  key={gameId}
                  className="bg-bg_dim flex flex-col justify-center md:mx-auto py-0 px-6 rounded-md w-full min-h-full max-w-md mt-2 text-xs"
                >
                  ...
                  <div className="flex items-center justify-between mb-2 text-xs">
                    {participants.map(({ image, name }) => (
                      <div
                        key={name}
                        className="flex items-center ml-2 first-of-type:ml-0"
                      >
                  ...
                  {isLock && (
                    <div className="text-bg-200 text-center">
          ...
          <div className="flex text-bg items-center text-bg  justify-between mt-1 px-2">
            ...
            <span className="text-sm font-semibold">
              {isOddsFetching ? <>Loading...</> : <>{totalOdds * +betAmount}</>}
            </span>
          </div>
          ...
          {account?.address ? (
            ...
            <div className="mt-1 flex justify-center items-center text-sm text-center">
              <p className="w-max py-2 px-3 bg-blue-600 rounded-2xl">
                Connect your wallet
              </p>
            </div>
          )}
